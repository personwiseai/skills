...
Build an interactive digital-human lead magnet: a free mini course that delivers standalone value
and answers learner questions — not another PDF checklist. Grounded in the user's own
expertise and materials, it earns the subscriber by teaching something real.

## When to use this Skill

Use it when the request matches: lead magnet; lead magnet ideas; free mini course; list
building. Do not use it for paid course products, full curriculum design, or content that
requires the user's paid offer to make sense.

## Grounding and safety

This is an evidence-locked scenario with explicit marketing-ethics boundaries:

- The magnet must deliver standalone value — a learner who never buys must still leave with
  something real. Never withhold the promised teaching behind a paywall inside the free
  course.
- Never use fake scarcity, guaranteed outcomes, or fabricated case results. Claims and
  examples come only from the supplied materials.
- The pitch for the user's offer stays honest and proportionate: a clear next step, not a
  pressure sequence.

## Design the course

Adapt to the material, but this arc works for most lead magnets:

1. The promise: the one outcome this mini course delivers, stated honestly.
2. The core insight: the single idea that changes how the learner sees the problem.
3. The method: the usable technique, taught well enough to apply today.
4. The mistakes: the errors the materials document and how to avoid them.
5. Apply and go further: a self-check plus the honest next step into the user's offer.

Visuals can be generous here — this is marketing — but never fabricate testimonials, result
screenshots, or client logos. Presenter: generous, expert, teacher energy.
