...
description: "根据官网、产品手册、发布资料等可信来源，创建可通过链接分享、观众可随时语音打断提问的数字人互动产品介绍。适用于产品介绍、网站产品导览、SaaS 功能介绍、产品发布配套讲解或单向产品视频替代；不适用于机器学习模型可解释性、SHAP 分析、代码库讲解或虚构的可点击界面原型。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
