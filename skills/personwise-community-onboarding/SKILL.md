...
Build an interactive digital-human onboarding course that turns new community members into
participating ones, grounded in the user's community guidelines, welcome materials, and
program documentation. The course is the "start here" that stays askable after the welcome
thread fades.

## When to use this Skill

Use it when the request matches: community onboarding; member onboarding; paid community
start here. Do not use it for employee or customer onboarding — those are different scenarios
— and do not use it for moderation policy drafting.
...
- Never invent member benefits, privacy rules, access levels, or perks the materials do not
  define — a community runs on trust, and wrong promises break it.
- Rules and norms are carried from the actual guidelines; etiquette suggestions beyond the
  written rules are presented as suggestions, never as policy.
- Disputes, access problems, and personal cases route to the community team named in the
  materials.

## Design the course

Adapt to the material, but this arc works for most community onboarding courses:

1. Welcome: what this community is for and who thrives here.
2. Your first week: the concrete first actions that make membership worth it.
3. How it works: the spaces, rituals, and programs — as documented.
4. The norms: the rules and the culture, honestly stated from the guidelines.
5. Self-check and people: confirm the essentials; who to ask for what.

Use supplied community imagery, event photos, or branding when available; otherwise keep
visuals warm and conceptual — never invent member counts, testimonials, or access tiers.
Presenter: warm, welcoming, community-host energy.
