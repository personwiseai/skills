---
name: personwise-lead-magnet-zh-cn
description: "当用户要求根据提供的资料制作引流磁铁小课时使用。触发语：引流磁铁；免费小课；名单增长；获客内容；互动演示。产出有依据支撑的数字人互动演示——观众可随时语音打断提问，以一门可提问的课程交付。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
用用户自己的专业积累和内容材料，制作一门数字人互动引流磁铁：一门交付独立价值、能回答
学习者提问的免费小课——不再是又一份 PDF 清单。靠真本事赢得订阅。

## 何时使用本 Skill

请求匹配以下表达时使用：引流磁铁；免费小课；名单增长；获客内容。不要用于付费课程
产品、完整课程体系设计，或离开用户付费产品就讲不通的内容。

## 依据与边界

本场景为证据锁定场景，并有明确的营销伦理边界：

- 磁铁必须交付独立价值——永远不买的学习者也要带走真东西。免费课程里绝不含着承诺的
  教学不放。
- 绝不用虚假稀缺、结果保证或不实案例。主张和案例只来自所提供的资料。
- 对用户产品的引荐保持诚实、适度：清晰的下一步，不是施压话术。

## 设计课程产物

按材料调整，大多数引流磁铁适用这条教学弧：

1. 承诺：这门小课交付的那一个成果，诚实地说。
2. 核心洞见：改变学习者看待问题方式的那一个观点。
3. 方法：可上手的方法，教到今天就能用。
4. 常见错误：资料记载的错误和规避方法。
5. 应用并继续：一次自检，加上进入用户产品的诚实下一步。

这里是营销场景，视觉可以慷慨——但绝不编造证言、结果截图或客户标识。讲解人基调：
慷慨、专业，老师气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-lead-magnet-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
