---
name: personwise-product-first-success-zh-cn
description: "当用户要求根据提供的资料制作产品首次成功指南时使用。触发语：产品设置；快速上手；快速入门指南；首次成功使用。产出有依据支撑、学习者可随时语音打断提问的数字人互动课程。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
用用户自己的设置指南、说明书和帮助文章，制作一门数字人互动快速上手课程，带新用户从
开箱或注册走到首次成功使用。课程缩短到达首次价值的时间，并回答卡住新用户的问题。

## 何时使用本 Skill

请求匹配以下表达时使用：产品设置；快速上手；快速入门指南；首次成功使用。不要用于
完整功能文档、高级管理配置或认证式培训。
...
- 绝不声称学习者完成了实际的安装、校准或安全操作——课程负责引导和检验理解，不执行、
  不认证现实世界操作。
- 高风险设备或受监管环境，课程必须明确要求现场或有资质人员确认后才能操作，并明说。
- 不得编造资料中没有的设置步骤、部件名称、耗时或兼容性说法。

## 设计课程产物

按材料调整，大多数首次成功课程适用这条教学弧：

1. 终点：首次成功长什么样，为什么值得花这段设置时间。
2. 开始前：需要什么、检查什么、这条路要走多久。
3. 引导路径：经过验证的最短路线，一步一步来。
4. 卡住时：最常见的几个堵点和文档记载的解法。
5. 验证并继续：确认结果；更深入的文档在哪里。

用户提供真实产品图、示意图或截图时使用；否则视觉保持概念化——绝不编造界面、接线或
硬件细节。讲解人基调：耐心、实用，现场指导员气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-product-first-success-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
