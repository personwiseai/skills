---
name: personwise-security-awareness-training-zh-cn
description: "当用户要求根据提供的资料制作安全意识培训时使用。触发语：安全意识培训；网络安全意识培训；员工安全培训。产出有依据支撑、学习者可随时语音打断提问的数字人互动课程。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
用用户自己的安全制度和意识培训材料，制作一门数字人互动安全意识课程。课程提升识别与
上报行为——它绝不声称能让任何人变得"安全"。

## 何时使用本 Skill

请求匹配以下表达时使用：安全意识培训；网络安全意识培训；员工安全培训。不要用于
技术安全运维培训、事件响应认证或渗透测试教学。

## 依据与边界

本场景为证据锁定场景。以下边界是硬边界，措辞不得软化：

- 绝不声称课程消除风险、保证安全、完成实操能力认证或替代安全团队——意识降低风险，
  不消除风险。
- 只教授资料中定义的制度、识别特征和上报路径；绝不编造威胁统计、泄露事件故事或攻击
  手法当作事实。
- 可疑事件一律指路到资料中具名的上报渠道，不即兴给处置建议。

## 设计课程产物

按材料调整，大多数意识课程适用这条教学弧：

1. 为什么重要：资料具名的威胁和它们的代价。
2. 识别：员工应该看出的特征，来自所提供制度。
3. 行动：文档记载的应对——该做什么、绝不做什么。
4. 上报：究竟向哪里、如何上报，之后会发生什么。
5. 自检：确认识别能力与上报条件反射。

视觉清晰、抓注意力，但不把编造的钓鱼截图冒充真实攻击；标注为示例的概念化示意可以
用。讲解人基调：警觉、实用，安全团队气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-security-awareness-training-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
