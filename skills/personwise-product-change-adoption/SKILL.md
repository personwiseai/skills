...
Build an interactive digital-human course that drives adoption of a specific product change — a
feature launch, a migration, or a breaking change — grounded in the user's release notes and
migration documentation. Users get the why, the how, and the deadline — and can ask the
course what the change means for them.

## When to use this Skill

Use it when the request matches: product update; feature adoption; migration guide; breaking
change — and there is a concrete change to teach. Do not use it for general product education
with no specific change trigger; that is ordinary course creation, which this Skill also
handles through the same workflow.
...
- Never promise compatibility, pricing, refunds, roadmap items, or migration outcomes the
  materials do not explicitly approve.
- Deadlines, deprecation dates, and required actions are carried exactly from the source;
  unclear dates are flagged for the user to confirm, never guessed.
- Account-specific migration questions route to the support or success channel named in the
  materials.
...
1. What changed and why: the change in one honest paragraph and the reason behind it.
2. What it means for you: who is affected, how, and by when — from the source only.
3. The migration path: the documented steps, in order, with effort honestly stated.
4. Edge cases: the documented caveats, breaking behaviors, and workarounds.
5. Self-check and help: confirm readiness; exactly where migration support lives.

Use real before/after imagery or screenshots when supplied; otherwise keep visuals conceptual
— never invent UI of the new feature. Presenter: clear, honest, release-notes-with-a-face
energy.
