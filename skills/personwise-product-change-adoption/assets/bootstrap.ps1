...
$upgradeHashes = @("151375f65df8b82e63ccba55119ad96badfc11437737452f67b1a274f229b4a5", "20a0dd088b2b19629899ee775b6bdf5189fe55ddd15765891ce76673169d53bb", "872e3b54d2d0262f682e319e45ad5b3daa1dcd1a3048d3cf12232694c5778cb6", "aa1c88ecf4e1fc0ea44c2e6a9d137c6f8361223737b3ddcbc00e82eb85e67b78")
$rollbackHashes = @()
$currentDigest = ""
...
try {
  $archive = Join-Path $workDir "personwise_1.1.2_windows_amd64.zip"
  ...
  $request = [Net.Http.HttpRequestMessage]::new([Net.Http.HttpMethod]::Get, "https://releases.personwise.ai/cli/v1.1.2/personwise_1.1.2_windows_amd64.zip")
  $response = $client.SendAsync($request, [Net.Http.HttpCompletionOption]::ResponseHeadersRead).GetAwaiter().GetResult()
  try {
    if (-not $response.IsSuccessStatusCode) { throw "Artifact download failed" }
    $declaredLength = $response.Content.Headers.ContentLength
    if ($null -ne $declaredLength -and ($declaredLength -ne 3112215 -or $declaredLength -gt 26214400)) { throw "Artifact size mismatch" }
    ...
    try {
      ...
      while (($read = $inputStream.Read($buffer, 0, $buffer.Length)) -gt 0) {
        $total += $read
        if ($total -gt 3112215 -or $total -gt 26214400) { throw "Artifact size limit exceeded during download" }
        $outputStream.Write($buffer, 0, $read)
      }
      if ($total -ne 3112215) { throw "Artifact size mismatch" }
  ...
  if ($file.Length -ne 3112215 -or $file.Length -gt 26214400) { throw "Artifact size mismatch" }
  $digest = (Get-FileHash -LiteralPath $archive -Algorithm SHA256).Hash.ToLowerInvariant()
  if ($digest -ne "8bfa70bf45c7ba20362f17b037b7ad33a3283572e89d8a1cd6dc2983cde9ca8e") { throw "Artifact checksum mismatch" }
  ...
  if ($candidateDigest -ne "8bec4b1137ce49e6e4692550a1e1454997a3a0097386e9ce690e7d053795f435") { throw "Release executable checksum mismatch" }
  ...
  [ordered]@{schema_version="1"; ok=$true; data=[ordered]@{path=[IO.Path]::GetFullPath($targetPath); software_version="1.1.2"; cli_contract_version="1.0"; action=$action}; request_id="bootstrap-local"} | ConvertTo-Json -Compress
} finally {
