---
name: personwise-sop-process-training-zh-cn
description: "当用户要求根据提供的资料制作 SOP 与流程培训时使用。触发语：SOP 培训；流程培训；操作规程培训；制度培训。产出有依据支撑、学习者可随时语音打断提问的数字人互动课程。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
用用户自己的 SOP 文件和制度文档，制作一门教授某一标准作业程序或流程的数字人互动课程。
课程把一份大家只会翻一遍的文件，变成可以提问的培训——并始终标明来源版本。

## 何时使用本 Skill

请求匹配以下表达时使用：SOP 培训；流程培训；操作规程培训；制度培训。不要用于监管
合规认证——那是有更严格规则的另一个场景——也不要用于从零起草新 SOP。

## 依据与边界

本场景为证据锁定场景：课程只教授所提供资料支持的内容。

- 课程必须标明它所教授的来源文件与精确版本，并说明：两者不一致时，以文件为准，
  课程不是权威。
- 绝不声称课程代替现场监督、执照、法定签署或必需的实操考核。
- 不得松散转述编号步骤、阈值或安全限值；严格按来源原文承载。

## 设计课程产物

按材料调整，大多数 SOP 课程适用这条教学弧：

1. 为什么有这个 SOP：它控制什么风险、保证什么结果。
2. 流程地图：先看全貌，再谈细节。
3. 操作程序：按顺序的步骤，以及关键的检查点和限值。
4. 例外与升级：什么时候停下，究竟找谁。
5. 自检与版本：确认理解；本课程教授的是哪个精确版本的 SOP。

视觉干净、程序化——流程图、检查清单。有提供的图示就用；绝不编造设备细节、阈值或
控制界面。讲解人基调：精确、冷静，质量负责人气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-sop-process-training-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
