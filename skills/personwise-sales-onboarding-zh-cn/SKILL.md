---
name: personwise-sales-onboarding-zh-cn
description: "当用户要求根据提供的资料制作销售入职培训时使用。触发语：销售入职培训；销售 Onboarding 项目；新销售上手。产出有依据支撑、学习者可随时语音打断提问的数字人互动课程。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
用用户已批准的销售手册和入职计划，制作一门数字人互动 Onboarding 课程，带新入职销售
走到最初几单。这是新销售 ramp 的专门场景——比泛销售赋能更窄。

## 何时使用本 Skill

只在请求匹配窄触发时使用：销售入职培训；销售 Onboarding 项目；新销售上手。当 broader
需求是销售就绪、GTM 培训或产品发布、且不聚焦新员工 ramp 时，路由到"销售与 GTM 就绪"
Skill。
...
- 绝不编造客户、业绩、竞争优势、财务结果或超出资料批准范围的主张。
- ramp 里程碑、业绩预期和流程规则只来自用户的入职计划——个人绩效问题指路到销售经理。
- 超出资料的产品、价格、合同问题，指路到具名负责人，不即兴作答。

## 设计课程产物

按材料调整，大多数销售入职课程适用这条教学弧：

1. 使命：我们卖什么、卖给谁、这个角色怎么赢。
2. 最初几周：ramp 计划、里程碑，以及早期"好"的标准。
3. 手册：已批准的故事、挖掘问题和证据点。
4. 流程：pipeline 阶段、工具和销售必须遵守的成交机制。
5. 演练与求助：讲一遍 pitch；什么事找谁。

视觉保持概念化——ramp 曲线、流程图——绝不编造 pipeline 截图、业绩表或客户名。讲解人
基调：直接、鼓劲，一线销售主管气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-sales-onboarding-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
