---
name: personwise-product-change-adoption-zh-cn
description: "当用户要求根据提供的资料制作产品变化采用课程时使用。触发语：产品更新；功能采用；迁移指南；破坏性变更；互动演示。产出有依据支撑的数字人互动演示——观众可随时语音打断提问，以一门可提问的课程交付。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
围绕一次具体的产品变化——新功能、迁移或破坏性变更——用用户的发布说明和迁移文档，
制作一门数字人互动采用课程。用户得到为什么、怎么办、deadline——并能向课程追问这个
变化对自己意味着什么。

## 何时使用本 Skill

请求匹配以下表达、且确有具体变化要教时使用：产品更新；功能采用；迁移指南；破坏性
变更。没有具体变化触发点的一般性产品教育不要使用——那是普通课程创建，本 Skill 用
同一工作流同样胜任。
...
- 绝不承诺资料未明确批准的兼容性、价格、退款、路线图或迁移结果。
- 时限、停用日期和必需动作严格按来源承载；不清楚的日期标注给用户确认，绝不猜。
- 账户特定的迁移问题，指路到资料中具名的支持或客户成功渠道。

## 设计课程产物

按材料调整，大多数变化采用课程适用这条教学弧：

1. 变了什么、为什么：一段诚实的话讲清变化和原因。
2. 对你意味着什么：谁受影响、怎么影响、什么时候之前——只按来源讲。
3. 迁移路径：文档记载的步骤，按顺序，成本如实说。
4. 边界情况：文档记载的注意事项、破坏性表现和变通办法。
5. 自检与求助：确认就绪；迁移支持究竟在哪里。

有真实的前后对比图或截图就用；否则视觉保持概念化——绝不编造新功能的界面。讲解人
基调：清楚、诚实，"长了脸的发布说明"气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-product-change-adoption-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
