---
name: personwise-end-user-software-training-zh-cn
description: "当用户要求根据提供的资料制作终端用户软件培训时使用。触发语：终端用户培训；软件使用培训；CRM 培训；ERP 用户培训。产出有依据支撑、学习者可随时语音打断提问的数字人互动课程。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
用用户自己的培训指南和流程文档，围绕某一业务软件系统，为终端用户——不是管理员，
不是开发者——制作一门数字人互动课程。这是组织内软件上手采用的专门场景。

## 何时使用本 Skill

只在请求匹配带终端用户限定的窄触发时使用：终端用户培训；软件使用培训；CRM 培训；
ERP 用户培训。当 broader 需求是组织变革或上线推广、且没有具体系统要教时，路由到
"内部变化采用"Skill。不要用于管理员、配置或开发者文档。

## 依据与边界

本场景为证据锁定场景：课程只教授所提供资料支持的内容。

- 绝不编造资料没有展示的界面细节、权限、功能或兼容性——培训里错一个点击路径，生产
  里就错一次操作。
- 课程负责引导和检验理解；绝不声称学习者已在真实系统里完成了实际操作。
- 界面随版本和租户变化：课程标明它所教授的版本，并指出以源指南为权威。

## 设计课程产物

按材料调整，大多数终端用户课程适用这条教学弧：

1. 为什么用这个系统：它干什么，学习者将负责哪些任务。
2. 全貌：主要区域和它们的关系——先建立方位感，再谈点击。
3. 核心工作流：最重要的任务，按源指南一步一步引导。
4. 常犯错误：文档记载的新手错误和恢复方法。
5. 自检与求助：确认工作流；支持和完整指南究竟在哪里。

所提供指南里的真实截图优先使用；否则视觉保持概念化——绝不编造界面。讲解人基调：
耐心、实用，资深用户气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-end-user-software-training-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
