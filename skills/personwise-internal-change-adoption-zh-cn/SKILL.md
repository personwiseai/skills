---
name: personwise-internal-change-adoption-zh-cn
description: "当用户要求根据提供的资料制作内部变革采用课程时使用。触发语：内部变革采用；组织变革；内部软件上线。产出有依据支撑、学习者可随时语音打断提问的数字人互动课程。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
用用户已批准的变革沟通材料，制作一门数字人互动课程，帮助员工接受一次具体的内部变化
——组织调整、新流程或软件上线。员工得到为什么、对我意味着什么、时间表——并能向
课程提出他们在全员会上不好意思问的问题。蓝图中显式 `distribution_target` 为 `private`，
不是公开链接。

## 何时使用本 Skill

请求匹配以下表达时使用：内部变革采用；组织变革；内部软件上线。不要用于抽象的变革
管理方法论培训——本场景教的是一次具体的、公司自己的变化。

## 依据与边界

本场景为证据锁定场景：课程只承载已批准的沟通内容。

- 绝不推测岗位结果、人事决定或未批准的未来状态——课程只承载已经沟通的内容，并诚实
  说明哪些还没有定论。
- 日期、范围和受影响团队严格按来源承载；模糊之处标注出来，不靠编造来"圆"。
- 个人影响问题，指路到具名渠道（直属经理、HR、变革团队），不即兴安抚。

## 设计课程产物

按材料调整，大多数变革采用课程适用这条教学弧：

1. 为什么要变：变革的真实理由，按已沟通的版本讲。
2. 对你意味着什么：按团队或角色的具体差别，只按来源讲。
3. 时间表：什么时候发生什么，每一步对员工的要求。
4. 支持：培训、资源和具名的求助渠道。
5. 自检与提问：确认要点；悬而未决的问题去哪里。

视觉冷静、结构化——时间表、前后对比图。绝不编造组织架构图、人数或未来状态的界面。
讲解人基调：诚实、稳定，变革负责人气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-internal-change-adoption-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
