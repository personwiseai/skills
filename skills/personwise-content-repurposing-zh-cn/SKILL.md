...
description: "当用户要求根据提供的资料把已有内容再利用成课程时使用。触发语：内容再利用；视频再利用；网络研讨会再利用；录像转课程；互动演示。产出有依据支撑的数字人互动演示——观众可随时语音打断提问，以一门可提问的课程交付。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。不限于本场景：任何其他课程创建请求都按同一工作流处理。"
license: MIT
