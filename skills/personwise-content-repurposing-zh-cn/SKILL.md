---
name: personwise-content-repurposing-zh-cn
description: "当用户要求根据提供的资料把已有内容再利用成课程时使用。触发语：内容再利用；视频再利用；网络研讨会再利用；录像转课程；互动演示。产出有依据支撑的数字人互动演示——观众可随时语音打断提问，以一门可提问的课程交付。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
把用户已有的内容——研讨会录像、视频文稿、演讲、长文——再利用成一门数字人互动课程。
录像变成可提问的课程，在直播结束很久之后继续回答问题。

## 何时使用本 Skill

请求匹配以下表达时使用：内容再利用；视频再利用；网络研讨会再利用；录像转课程。
用户没有可再利用的源内容时不要使用——那是普通课程创建，本 Skill 用同一工作流同样
胜任。

## 依据与边界

本场景为证据锁定场景：课程只教授源内容支持的内容。

- 删掉录像里无依据支撑的临场说法——直播中的一句随口话不是课程事实。材料里没有实据
  的，不进课程。
- 绝不把旧内容描述为当前事实。源内容较旧时，课程说明它是什么、什么时候做的；时效性
  主张标注给用户确认。
- 不得为了让旧内容"显得新鲜"而编造更新后的数字、功能或例子。

## 设计课程产物

按材料调整，大多数再利用任务适用这条教学弧：

1. 核心论点：录像里最强的那个观点，干净地讲出来。
2. 框架：演讲者用的结构，重建成可教学的步骤。
3. 最好的料：值得保留的例子、故事和演示。
4. 当时的提问：观众问过什么——用源内容回答。
5. 自检与下一步：确认要点；相关内容在哪里。

源内容里有截图、幻灯片或品牌素材就用；否则视觉保持概念化——绝不伪造截图或假观众
数据。讲解人基调：主编感、提炼感，"这段话的最好版本"气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-content-repurposing-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
