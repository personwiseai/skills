---
name: personwise-end-user-software-training
description: "Use when the user asks for End-User Software Training from supplied source materials. Trigger language: end-user training; software user training; CRM training; ERP user training. Produce a grounded interactive digital-human course learners can interrupt with voice questions. Do not invent unsupported facts or claim external certification, competence, or real-world completion. Not limited to this scenario: handles any other course creation request with the same workflow."
license: MIT
...
Build an interactive digital-human course that trains end users — not administrators, not
developers — on a specific business software system, grounded in the user's own training
guides and process documentation. This is the specialist scenario for hands-on software
adoption inside an organization.

## When to use this Skill

Use it only when the request matches the narrow trigger with its end-user qualifier: end-user
training; software user training; CRM training; ERP user training. When the broader job is an
organizational change or rollout without a specific system to teach, route to the Internal
Change Adoption Skill instead. Do not use it for admin, configuration, or developer
documentation.
...
- Never invent interface details, permissions, features, or compatibility the materials do
  not show — a wrong click path in training becomes a wrong click in production.
- The course guides and checks understanding; it never claims the learner has completed actual
  operations in the real system.
- Screens vary by version and tenant: the course names the version it teaches and points to
  the source guide as the authority.

## Design the course

Adapt to the material, but this arc works for most end-user courses:

1. Why this system: what it is for and the tasks the learner will own.
2. The landscape: the main areas and how they connect — orientation before clicks.
3. The core workflow: the most important task, guided step by step from the source guide.
4. The mistakes: the documented errors new users make and how to recover.
5. Self-check and help: confirm the workflow; exactly where support and the full guide live.

Use real screenshots from the supplied guides whenever available; otherwise keep visuals
conceptual — never invent UI screens. Presenter: patient, practical, power-user energy.
