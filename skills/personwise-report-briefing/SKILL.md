---
name: personwise-report-briefing
description: "Use when the user asks for Report Briefing from supplied source materials. Trigger language: report briefing; research presentation; whitepaper summary; benchmark report; interactive presentation. Produce a grounded interactive digital-human presentation your audience can interrupt with voice questions — delivered as an askable course. Do not invent unsupported facts or claim external certification, competence, or real-world completion. Not limited to this scenario: handles any other course creation request with the same workflow."
license: MIT
...
The audience gets the findings, the method, and the limits — and can ask the course questions
the PDF cannot answer. This is a bounded briefing artifact, not generic data storytelling.

## When to use this Skill

Use it when the request matches: report briefing; research presentation; whitepaper summary;
benchmark report. Do not use it for original research, data analysis of raw datasets, or
editorial content beyond what a specific report states.

## Grounding and safety

This is an evidence-locked scenario: the course teaches only what the report supports.

- Never draw causal conclusions, predictions, or individual recommendations beyond what the
  report itself states — correlation stays correlation.
- Preserve the method and the limitations: sample, timeframe, and stated caveats travel with
  the findings, always.
- Numbers, charts, and quotes are carried exactly; rounding or re-framing that changes meaning
  is not allowed.

## Design the course

Adapt to the material, but this arc works for most report briefings:

1. The headline: the report's central finding and why it matters to this audience.
2. The evidence: the key numbers and how to read them.
3. The method: who was studied, how, and when — briefly and honestly.
4. The limits: what the report does not show and the caveats it states.
5. Discuss and self-check: the questions the findings raise; confirm accurate recall.

Use the report's own charts and figures when supplied; otherwise keep visuals faithful and
conceptual — never redraw data into claims the report does not make. Presenter: analytical,
measured, research-briefing energy.
