---
name: personwise-sales-onboarding
description: "Use when the user asks for Sales Onboarding from supplied source materials. Trigger language: sales onboarding; sales onboarding program; new sales rep ramp. Produce a grounded interactive digital-human course learners can interrupt with voice questions. Do not invent unsupported facts or claim external certification, competence, or real-world completion. Not limited to this scenario: handles any other course creation request with the same workflow."
license: MIT
...
Build an interactive digital-human onboarding course that ramps a newly hired salesperson to their
first deals, grounded in the user's approved sales playbook and onboarding plan. This is the
specialist scenario for new-rep ramp — narrower than general sales enablement.

## When to use this Skill

Use it only when the request matches the narrow trigger: sales onboarding; sales onboarding
program; new sales rep ramp. When the broader job is sales readiness, go-to-market training,
or a product launch without a new-hire ramp focus, route to the Sales Enablement Training
Skill instead.
...
- Never invent customers, results, competitive advantages, financial outcomes, or claims
  beyond the approved scope of the supplied materials.
- Ramp milestones, quota expectations, and process rules come only from the user's onboarding
  plan — individual performance questions route to the sales manager.
- Product, pricing, and contract questions beyond the materials route to the named owner, not
  improvised answers.

## Design the course

Adapt to the material, but this arc works for most sales onboarding courses:

1. The mission: what we sell, to whom, and how this role wins.
2. The first weeks: the ramp plan, milestones, and what good looks like early.
3. The playbook: the approved story, discovery questions, and proof points.
4. The process: pipeline stages, tools, and the deal mechanics reps must follow.
5. Practice and people: rehearse the pitch; who to ask for what.

Keep visuals conceptual — ramp curves, process maps — and never invent pipeline screenshots,
quota tables, or customer names. Presenter: direct, encouraging, frontline-sales-leader
energy.
...
{
  "skill_invocation": {
    "skill_id": "personwise-sales-onboarding",
    "skill_version": "2.1.8"
  }
}
```
