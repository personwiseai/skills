---
name: personwise-developer-quickstart-zh-cn
description: "当用户要求根据提供的资料制作开发者快速上手课程时使用。触发语：开发者快速上手；API 教程；SDK 教程；首次成功 API 调用。产出有依据支撑、学习者可随时语音打断提问的数字人互动课程。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
用用户自己的 API 文档、SDK 指南和参考材料，制作一门数字人互动快速上手课程，带开发者
从拿到凭据走到首次成功的 API 调用。这是对外技术采用——终点是跑通第一次调用，不是
内部团队 ramp。

## 何时使用本 Skill

请求匹配以下表达时使用：开发者快速上手；API 教程；SDK 教程；首次成功 API 调用。
不要用于内部开发者 ramp 或团队 Onboarding——那是另一个场景——也不要用于完整 API
参考文档。

## 依据与边界

本场景为证据锁定场景，并有严格的可验证性要求。

- 代码、端点、参数和版本必须能对照所提供文档验证。代码块原样承载，绝不即兴编语法。
- 绝不编造资料中没有的端点、参数、认证方式、兼容性说法或安全保证。
- 标明本课程所教授文档的版本或日期，并指出：如有变化，以源文档为权威。

## 设计课程产物

按材料调整，大多数快速上手课程适用这条教学弧：

1. 你要做出什么：以首次成功调用为终点，以及它为什么重要。
2. 环境准备：凭据、环境，以及本课程覆盖的已验证版本。
3. 第一次调用：最小可用请求，逐行讲清楚。
4. 读懂响应：成功返回什么，常见错误是什么意思。
5. 复现并深入：确认能自己重复；完整参考文档在哪里。

优先使用所提供文档里的真实代码块，而不是任何配图；图示保持概念化（请求流、认证流），
绝不编造控制台界面或仪表盘。讲解人基调：精确，开发者对开发者气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-developer-quickstart-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
