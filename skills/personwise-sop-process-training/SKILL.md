---
name: personwise-sop-process-training
description: "Use when the user asks for SOP and Process Training from supplied source materials. Trigger language: SOP training; process training; procedure training; policy training. Produce a grounded interactive digital-human course learners can interrupt with voice questions. Do not invent unsupported facts or claim external certification, competence, or real-world completion. Not limited to this scenario: handles any other course creation request with the same workflow."
license: MIT
...
Build an interactive digital-human course that teaches one standard operating procedure or process,
grounded in the user's own SOP documents and policy files. The course turns a document people
skim into training they can question — with the source version always visible.

## When to use this Skill

Use it when the request matches: SOP training; process training; procedure training; policy
training. Do not use it for regulatory compliance certification — that is a separate scenario
with stricter rules — and do not use it to draft new SOPs from nothing.
...
- The course must name the exact source document and version it teaches, and must say the
  document — not the course — is the authority when they differ.
- Never claim the course replaces on-site supervision, licenses, statutory sign-off, or
  required practical assessment.
- Do not paraphrase numbered steps, thresholds, or safety limits loosely; carry them exactly
  as the source states them.

## Design the course

Adapt to the material, but this arc works for most SOP courses:

1. Why this SOP exists: the risk it controls and the outcome it guarantees.
2. The process map: the whole flow at a glance before any detail.
3. The procedure: the steps in order, with the checks and limits that matter.
4. Exceptions and escalation: when to stop, and exactly who to call.
5. Self-check and version: confirm understanding; the exact SOP version this course teaches.

Keep visuals clean and procedural — flow diagrams and checklists. Use supplied figures when
available; never invent equipment detail, thresholds, or control screens. Presenter: precise,
calm, quality-lead energy.
