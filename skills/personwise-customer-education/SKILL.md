...
Build an interactive digital-human customer education course around one high-frequency question
cluster, grounded in the user's own help center articles, product documentation, or support
playbooks. The course teaches customers to solve the problem themselves and answers their
follow-up questions.

## When to use this Skill

Use it when the request matches: customer education; customer training; customer academy;
support deflection. Do not use it for account-specific troubleshooting or diagnosis — that
belongs to human support.
...
- Never pretend to resolve account-specific faults; anything needing diagnosis or human
  handling escalates to the user's support channel.
- When a question inside the course scope has no support in the materials, say so and point to
  support; do not improvise fixes.
- Present the course as education, not as a support SLA, diagnostics, or a guarantee of issue
  resolution.

## Design the course

Adapt to the material, but this arc works for most question clusters:

1. The problem: when it appears, what it costs, and what success looks like.
2. The mental model: the one concept that makes the fix make sense.
3. The guided workflow: the standard resolution path, step by step.
4. Pitfalls and edge cases: the frequent mistakes and the FAQ.
5. Self-check and escalation: confirm understanding; exactly when and how to reach support.

Prefer real UI screenshots or supplied product images when the user provides them; otherwise
keep visuals conceptual rather than invented UI. Presenter: calm, practical,
support-engineer energy.
