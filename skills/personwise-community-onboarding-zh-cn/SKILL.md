---
name: personwise-community-onboarding-zh-cn
description: "当用户要求根据提供的资料制作社区 Onboarding 课程时使用。触发语：社区 Onboarding；会员 Onboarding；付费社区入门。产出有依据支撑、学习者可随时语音打断提问的数字人互动课程。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
用用户的社区守则、欢迎材料和项目文档，制作一门数字人互动 Onboarding 课程，把新会员
变成会参与的会员。课程是那个"从这里开始"，在欢迎帖沉下去之后依然可问。

## 何时使用本 Skill

请求匹配以下表达时使用：社区 Onboarding；会员 Onboarding；付费社区入门。不要用于
员工或客户 Onboarding——那是另外两个场景——也不要用于起草管理政策。

## 依据与边界

本场景为证据锁定场景：课程只教授所提供资料支持的内容。

- 绝不编造资料没有定义的会员权益、隐私规则、访问级别或特权——社区靠信任运转，错误
  的承诺会打破它。
- 规则与规范来自真实的守则；超出成文规则的建议，作为建议呈现，绝不冒充政策。
- 纠纷、访问问题和个案，指路到资料中具名的社区团队。

## 设计课程产物

按材料调整，大多数社区入门课程适用这条教学弧：

1. 欢迎：这个社区为了什么存在，谁在这里如鱼得水。
2. 你的第一周：让会员资格值回票价的几个具体动作。
3. 怎么运转：版块、仪式和项目——按文档讲。
4. 规范：规则与文化，按守则诚实陈述。
5. 自检与求助：确认要点；什么事找谁。

有提供的社区图片、活动照片或品牌素材就用；否则视觉保持温暖、概念化——绝不编造会员
数量、证言或访问层级。讲解人基调：温暖、欢迎，社区主理人气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-community-onboarding-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
