---
name: personwise-student-orientation-zh-cn
description: "当用户要求根据提供的资料制作新生 Orientation 课程时使用。触发语：新生 Orientation；新生入学教育；国际学生 Orientation。产出有依据支撑、学习者可随时语音打断提问的数字人互动课程。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
用学校或机构自己的学生手册、Orientation 材料和制度文档，为 incoming 新生制作一门
数字人互动 Orientation 课程。每一届新生都问同样的问题，课程在开学前就回答，并在之后
保持可问。

## 何时使用本 Skill

请求匹配以下表达时使用：新生 Orientation；新生入学教育；国际学生 Orientation。
不要用于学术课程内容、录取决定或学生个案。

## 依据与边界

本场景为证据锁定场景：课程只教授所提供资料支持的内容。

- 绝不编造或猜测学籍、费用、签证规则、住宿、录取条件、无障碍安排或安全政策——严格
  按所提供手册承载，资料没覆盖就说没覆盖。
- 任何个案性或后果重大的事项（签证个案、费用争议、无障碍申请），指路到资料中具名
  的办公室，不即兴指导。
- 政策随届别变化：课程标明它所教授的来源文件与年份。

## 设计课程产物

按材料调整，大多数 Orientation 课程适用这条教学弧：

1. 欢迎：这是什么地方，最初几天长什么样。
2. 必需品：每个学生都需要的账号、文件和注册事项。
3. 日常：作息、地点和服务怎么运转。
4. 重要的规则：学生最常踩的政策，按手册原文讲。
5. 自检与求助：确认要点；哪类问题究竟找哪个办公室。

有提供的校园或项目图片就用；否则视觉保持亲切、概念化——绝不编造校园地图、房间细节
或具名教职工。讲解人基调：温暖、让人安心，学生服务老师气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-student-orientation-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
