---
name: personwise-customer-education-zh-cn
description: "当用户要求根据提供的资料制作客户教育课程时使用。触发语：客户教育；客户培训；客户学院；支持分流。产出有依据支撑、学习者可随时语音打断提问的数字人互动课程。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
围绕一个高频问题簇，用用户自己的帮助中心文章、产品文档或支持手册，制作一门数字人互动
客户教育课程。课程教会客户自己解决问题，并回答他们的追问。

## 何时使用本 Skill

请求匹配以下表达时使用：客户教育；客户培训；客户学院；支持分流。不要用于账户特定的
故障排查或诊断——那属于人工支持。
...
- 绝不假装解决账户特定故障；任何需要诊断或人工处理的问题，升级到用户的支持渠道。
- 课程范围内的问题在资料中没有支撑时，如实说明并指向支持渠道；不得即兴编方案。
- 把课程定位为教育，而不是支持 SLA、诊断服务或问题解决的保证。

## 设计课程产物

按材料调整，大多数问题簇适用这条教学弧：

1. 问题本身：什么时候出现、代价是什么、解决成功是什么样。
2. 心智模型：让解法讲得通的那一个核心概念。
3. 引导式流程：标准解决路径，一步一步来。
4. 坑与边界情况：最常犯的错误和 FAQ。
5. 自检与升级：确认理解；究竟何时、如何联系支持。

用户提供真实 UI 截图或产品图时优先使用；否则视觉保持概念化，不编造界面。讲解人基调：
沉稳、实用，支持工程师气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-customer-education-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
