---
name: personwise-customer-onboarding-zh-cn
description: "当用户要求根据提供的资料制作客户 Onboarding 课程时使用。触发语：客户 Onboarding；客户启用培训；新客户培训；实施启动。产出有依据支撑、学习者可随时语音打断提问的数字人互动课程。不得编造无依据的事实，不得声称外部认证、能力资质或现实任务已完成。在用户明确同意后，本技能会安装或更新官方 PersonWise CLI 并使用浏览器 OAuth。其他课程创建请求仅在该用户明确使用本技能时处理。"
license: MIT
...
用用户已确认的 Onboarding 材料，制作一门数字人互动课程，带新客户从注册走到首次价值。
每一个新客户都会重新触发同样的需求，一门好课程服务整条客流。

## 何时使用本 Skill

请求匹配以下表达时使用：客户 Onboarding；客户启用培训；新客户培训；实施启动。不要用
于员工或新员工 Onboarding——那是另一个场景——也不要用于账户特定的实施项目管理。

## 依据与边界

本场景为证据锁定场景，建立在已确认的承诺之上。

- 只呈现已确认的范围、承诺、时间表和支持渠道。资料没有确认的承诺，课程就不做。
- 不得编造实施日期、SLA 条款、价格或具名联系人。
- 关于某个具体客户合同或套餐的问题，回到用户的团队解答。

## 设计课程产物

按材料调整，大多数 Onboarding 课程适用这条教学弧：

1. 欢迎与终点：首次价值长什么样、要走多远。
2. 地图：Onboarding 旅程一览——阶段、里程碑、客户要做什么。
3. 第一个胜利：最快交付价值的那一个引导动作。
4. 节奏：接下来发生什么、典型时间表、如何保持不卡壳。
5. 帮助与升级：支持渠道、响应预期、去哪里找答案。

视觉让人安心、具体；有真实产品图就用，绝不编造界面或假仪表盘。讲解人基调：温暖、
有条理，客户成功经理气质。
...
{
  "skill_invocation": {
    "skill_id": "personwise-customer-onboarding-zh-cn",
    "skill_version": "2.1.2"
  }
}
```
