...
Build an interactive digital-human quick-start course that takes a new user from unboxing or signup
to their first successful use, grounded in the user's own setup guides, manuals, and help
articles. The course shortens time-to-first-value and answers the questions that stall new
users.

## When to use this Skill

Use it when the request matches: product setup; getting started; quick start guide; first
successful use. Do not use it for full feature documentation, advanced administration, or
...
- Never claim the learner has completed an actual installation, calibration, or safety
  operation — the course guides and checks understanding, it does not perform or certify
  real-world actions.
- For high-risk equipment or regulated environments, the course must require on-site or
  qualified-person confirmation before operation; say so explicitly.
- Do not invent setup steps, part names, timings, or compatibility claims that the materials
  do not state.

## Design the course

Adapt to the material, but this arc works for most first-success courses:

1. The destination: what first success looks like and why it is worth the setup time.
2. Before you start: what you need, what to check, and how long the path takes.
3. The guided path: the shortest verified route to first success, step by step.
4. When it stalls: the most common blockers and their documented fixes.
5. Verify and continue: confirm the result; where the deeper documentation lives.

Use real product imagery, diagrams, or screenshots when the user supplies them; otherwise keep
visuals conceptual — never invent UI, wiring, or hardware detail. Presenter: patient,
practical, field-guide energy.
